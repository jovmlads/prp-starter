...
Write-Host "`n📝 Now you need to update .env.local with your Supabase credentials:`n" -ForegroundColor Yellow
...
Write-Host "5. Edit .env.local and replace the placeholder values`n" -ForegroundColor White

Write-Host "📋 Your .env.local file is ready to edit:" -ForegroundColor Cyan
Write-Host "   $((Get-Location).Path)\.env.local`n" -ForegroundColor White
...
if (Test-Path "database\schema.sql") {
    ...
    Write-Host "⚠️  Database schema not found. Make sure to run the schema in Supabase." -ForegroundColor Yellow
}
...
Write-Host "`n⚡ Once configured, run: npm run dev`n" -ForegroundColor Green
